...
export default function LeadDetail({ leadId }) {
  ...
  useEffect(() => {
    if (leadId) {
      ...
      const channels = [
        supabase
          .channel(`lead-${leadId}-realtime`)
          .on('postgres_changes', { event: '*', schema: 'public', table: 'leads', filter: `id=eq.${leadId}` }, () => fetchLead())
          .subscribe(),
        supabase
          .channel(`interactions-${leadId}-realtime`)
          .on('postgres_changes', { event: '*', schema: 'public', table: 'interactions', filter: `lead_id=eq.${leadId}` }, () => fetchInteractions())
          .subscribe(),
      ]
