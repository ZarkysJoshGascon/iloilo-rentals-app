...
import {
  ...
  isSameDay, isWithinInterval, parseISO, getDay, addMonths
} from 'date-fns'
import {
  ChevronDown, Home, Calendar, Mail, Hash, Wrench,
  Users, FileText, X, AlertTriangle, Check, Clock
} from 'lucide-react'
...
function isLocalHoliday(date) {
  ...
  if (month === 0 && dayOfWeek === 0 && isSameDay(date, getNthSunday(year, 0, 4)))
    return { name: 'Dinagyang Festival', scope: 'local', type: 'festival' }
...
function isWorldwideHoliday(date) {
  const year = date.getFullYear(), month = date.getMonth(), dayOfWeek = date.getDay()
  const key = format(date, 'MM-dd')
  if (WORLDWIDE_HOLIDAYS[key]) return WORLDWIDE_HOLIDAYS[key]
  if (month === 4 && dayOfWeek === 0 && isSameDay(date, getNthSunday(year, 4, 2)))
    return { name: "Mother's Day", scope: 'global', type: 'observance' }
  if (month === 5 && dayOfWeek === 0 && isSameDay(date, getNthSunday(year, 5, 3)))
    return { name: "Father's Day", scope: 'global', type: 'observance' }
  return null
}
...
function ContextMenu({ x, y, onClose, condo, onAddEvent }) {
  ...
  return (
    <motion.div
      ...
      ref={menuRef}
      className="fixed z-[9999] bg-white dark:bg-gray-800 rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.2)] border border-gray-200 dark:border-gray-600 py-2 min-w-[240px] backdrop-blur-sm"
      style={{ left: x, top: y }}
    >
      ...
      {Object.entries(EVENT_TYPES).map(([key, type]) => (
        <button
          key={key}
          onClick={() => { onAddEvent(key); onClose() }}
          className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors text-left"
        >
          ...
          <div>
            <p className="font-semibold text-gray-800 dark:text-gray-100">{type.label}</p>
            <p className="text-[10px] text-gray-400 dark:text-gray-500">Add {type.label.toLowerCase()} event</p>
          </div>
        </button>
      ))}
    </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.92, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 30 }} transition={{ type: "spring", damping: 25, stiffness: 300 }}
            ...
            <div className="relative px-6 py-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white tracking-tight">New {typeConfig.label}</h3>
                    <p className="text-white/70 text-sm mt-0.5 font-medium">{condo?.title}</p>
                  </div>
                </div>
          ...
          <div className="p-6 space-y-5">
            {isSingleDate ? (
              <div className="bg-gray-50 dark:bg-gray-700/50 rounded-2xl p-4">
                <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">
                  {eventType === 'contract_end' ? 'Contract End Date' : 'Meeting Date'}
                </label>
                <input
                  type="date"
                  value={singleDate}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-50 dark:bg-gray-700/50 rounded-2xl p-4">
                  <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Start Date</label>
                  <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full border-2 border-gray-200 dark:border-gray-600 rounded-xl px-4 py-3 text-sm bg-white dark:bg-gray-700 dark:text-gray-100 font-semibold focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" />
                </div>
                <div className="bg-gray-50 dark:bg-gray-700/50 rounded-2xl p-4">
                  <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">End Date</label>
                  <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full border-2 border-gray-200 dark:border-gray-600 rounded-xl px-4 py-3 text-sm bg-white dark:bg-gray-700 dark:text-gray-100 font-semibold focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" />
                </div>
              </div>
            )}
                ...
                <div className="grid grid-cols-4 gap-2">
                  {COLOR_PRESETS.map((preset) => (
                    <button
                      ...
                      style={{
                        backgroundColor: customColor === preset.color ? preset.color + '20' : 'transparent',
                        border: customColor === preset.color ? `2px solid ${preset.color}` : '2px solid transparent'
                      }}
                    >
            ...
            <div className="flex gap-3 pt-1">
              <button onClick={onClose} className="flex-1 px-5 py-3.5 border-2 border-gray-200 dark:border-gray-600 rounded-2xl text-gray-600 dark:text-gray-300 font-bold text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-all">
                Cancel
              </button>
              <button
                onClick={handleSave}
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl hover:border-gray-300 dark:hover:border-gray-500 transition-all shadow-sm"
      >
        <Calendar size={15} className="text-[#2d568e] dark:text-blue-400" />
        <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">
          {MONTHS[currentMonth ? currentMonth.getMonth() : pickerMonth]} {currentMonth ? currentMonth.getFullYear() : pickerYear}
        </span>
        ...
        {isOpen && (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <button onClick={() => setPickerYear(y => y - 1)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500">
            ...
            <div className="grid grid-cols-3 gap-2">
              {MONTHS.map((month, idx) => (
                <button
                  key={month}
                  onClick={() => { setPickerMonth(idx); handleSelect(pickerYear, idx) }}
                  className={`px-2 py-2.5 rounded-xl text-sm font-medium transition-all ${
                    idx === pickerMonth ? 'bg-[#2d568e] text-white shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'
                  }`}
                >
                  {month.substring(0, 3)}
                </button>
...
export default function CalendarView() {
  ...
  const [addEventModal, setAddEventModal] = useState({
    isOpen: false, eventType: null, startDate: null, endDate: null
  })

  // Initialise visible months without effect
  ...
  const fetchData = useCallback(async () => {
    ...
    const [condosRes, bookingsRes, eventsRes] = await Promise.all([
      ...
      supabase.from('bookings').select('id, condo_id, start_date, end_date, status, guest_name, guest_email, booking_code, avatar_url')
    ...
    try {
      ...
      for (const year of [currentYear - 1, currentYear, currentYear + 1]) {
        ...
        if (response.ok) {
          ...
          allHolidays.push(...data.map(h => ({ ...h, scope: 'national' })))
        }
      }
  ...
  const handleDragEnd = useCallback(() => {
    ...
    setDragStart(null)
    setDragEnd(null)
  }, [dragStart, dragEnd, selectedCondo])
  ...
  // Mouse move listener for tooltip positioning
  useEffect(() => {
  ...
  // Global mouse up to end drag even if released outside
  useEffect(() => {
  ...
  const getBookingsForDay = (condoId, day) => {
    ...
    return bookings.filter(b => {
      const start = parseISO(b.start_date), end = parseISO(b.end_date)
      return b.condo_id === condoId && isWithinInterval(day, { start, end })
    })
  }
  ...
  const getEventsForDay = (condoId, day) => {
    ...
    return events.filter(e => {
      ...
      const end = e.end_date ? parseISO(e.end_date) : parseISO(e.start_date)
      return e.condo_id === condoId && isWithinInterval(day, { start, end })
    })
  }
  ...
  const hasEventConflict = (booking) => {
    ...
    return events.some(e => {
      const eStart = parseISO(e.start_date), eEnd = e.end_date ? parseISO(e.end_date) : parseISO(e.start_date)
      return e.condo_id === booking.condo_id && bStart <= eEnd && bEnd >= eStart
    })
  }
  ...
  const getHolidayForDay = (day) => {
    ...
    const local = isLocalHoliday(day)
    if (local) return { ...local, date: dateStr }
    const world = isWorldwideHoliday(day)
    if (world) return { ...world, date: dateStr }
    return null
  }
      ...
      window.requestAnimationFrame(() => {
        ...
        Object.entries(monthRefs.current).forEach(([index, el]) => {
          if (el) {
            ...
            if (distance < closestDistance) { closestDistance = distance; closestMonth = parseInt(index) }
          }
        })
  ...
  const navigateToDate = (date) => {
    ...
    if (idx >= 0 && monthRefs.current[idx]) {
  ...
  const handleEventSaved = () => {
    fetchData()
    setAddEventModal({ isOpen: false, eventType: null, startDate: null, endDate: null })
  }
  ...
  /* ---------- main render ---------- */
  return (
    <div className="flex gap-5 h-full font-sans">
      {/* LEFT SIDE – Listings */}
      <div className="w-[280px] flex-shrink-0 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-600 overflow-hidden shadow-sm flex flex-col">
        <div className="px-5 py-4 bg-[#2d568e]">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white tracking-wide">Listings</h3>
            <span className="text-[10px] font-semibold text-white/70 bg-white/15 px-2.5 py-1 rounded-full">{condos.length} units</span>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {condos.map((condo) => {
            const imageUrl = getCondoImage(condo)
            const isSelected = selectedCondo?.id === condo.id
            return (
              <motion.button
                key={condo.id}
                onClick={() => setSelectedCondo(condo)}
                onContextMenu={(e) => handleContextMenu(e, condo)}
                className={`w-full flex items-start gap-3 px-4 py-3.5 transition-all text-left border-b border-gray-50 dark:border-gray-700/50 ${
                  isSelected
                    ? 'bg-[#2d568e]/10 dark:bg-blue-900/30 border-l-[4px] border-l-[#2d568e]'
                    : 'border-l-[4px] border-l-transparent hover:bg-gray-50 dark:hover:bg-gray-700/30'
                }`}
              >
                <div className="w-11 h-11 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100 dark:bg-gray-700 shadow-sm border border-gray-200 dark:border-gray-600">
                  {imageUrl && !imgErrors[condo.id] ? (
                    <img src={imageUrl} alt={condo.title} className="w-full h-full object-cover" onError={() => setImgErrors(prev => ({ ...prev, [condo.id]: true }))} />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/20">
                      <Home size={20} className="text-[#2d568e] dark:text-blue-400" />
                    </div>
                  )}
                </div>
                <div className="min-w-0 flex-1 pt-0.5">
                  <p className={`text-[13px] font-semibold truncate leading-tight ${isSelected ? 'text-[#2d568e] dark:text-blue-400' : 'text-gray-900 dark:text-gray-100'}`}>
                    {condo.title}
                  </p>
                  {condo.code && (
                    <p className="text-[10px] text-gray-400 dark:text-gray-500 font-mono font-semibold mt-0.5 tracking-wide">{condo.code}</p>
                  )}
                </div>
              </motion.button>
            )
        ...
        {selectedCondo ? (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-600 overflow-hidden shadow-sm flex flex-col h-full">
            ...
            <div className="border-b border-gray-100 dark:border-gray-700 bg-gray-50/30 dark:bg-gray-700/20 flex-shrink-0">
              ...
              <div className="px-5 py-3 flex items-center justify-between">
                <button onClick={goToToday} className="px-4 py-2 bg-[#2d568e] text-white rounded-xl text-[13px] font-semibold hover:bg-[#1e3a5f] transition-all shadow-sm tracking-wide">
                  Today
                </button>
                <div className="text-center">
                  <p className="text-lg font-semibold text-gray-900 dark:text-gray-100 tracking-tight">
                    {visibleMonths[activeMonthIndex] ? format(visibleMonths[activeMonthIndex], 'MMMM yyyy') : ''}
                  </p>
                </div>
              ...
              <div className="px-5 pb-2.5 flex items-center justify-center gap-4 text-[10px] font-semibold">
                <div className="flex items-center gap-1">
                  <div className="w-3.5 h-2.5 rounded-full bg-[#0f766e]"></div>
                  <span className="text-gray-500 dark:text-gray-400">Booked</span>
                </div>
                <div className="flex items-center gap-1">
                  <div className="w-3.5 h-2.5 rounded-full bg-[#374151]"></div>
                  <span className="text-gray-500 dark:text-gray-400">Maintenance</span>
                </div>
                <div className="flex items-center gap-1">
                  <div className="w-3.5 h-2.5 rounded-full bg-[#1d4ed8]"></div>
                  <span className="text-gray-500 dark:text-gray-400">Meeting</span>
                </div>
                <div className="flex items-center gap-1">
                  <div className="w-3.5 h-2.5 rounded-full bg-[#dc2626]"></div>
                  <span className="text-gray-500 dark:text-gray-400">Contract End</span>
                </div>
              </div>
            </div>
                  ...
                  <div key={monthIdx} ref={(el) => { monthRefs.current[monthIdx] = el }}>
                    <div className="mb-4 pb-2.5 border-b border-gray-100 dark:border-gray-700">
                      <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 tracking-tight">
                        ...
                        {isCurrentMonth && (
                          <span className="ml-3 text-[10px] font-semibold text-[#2d568e] dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded-full tracking-wide">
                            Current
                          </span>
                        )}
                      </h3>
                      ...
                      {days.map((day) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {booking && (
                              <div
                                onMouseEnter={() => handleMouseEnter(booking, 'booking')}
                                onMouseLeave={handleMouseLeave}
                                className={`absolute z-20 transition-all hover:brightness-110 flex items-center justify-center ${
                                  isBStart && isBEnd
                                    ? 'inset-x-0.5 rounded-full'
                                    : isBStart
                                    ? 'left-0.5 -right-px rounded-l-full'
                                    : isBEnd
                                    ? '-left-px right-0.5 rounded-r-full'
                                    : '-left-px -right-px'
                              ...
                              >
                                {isEStart && (
                                  <div className="flex items-center gap-1 px-1">
                                    {(() => {
                                      const Icon = EVENT_TYPES[event.event_type]?.icon || Calendar
                                      return <Icon size={12} className="text-white flex-shrink-0" />
                                    })()}
                            ...
                            <span
                              className={`absolute top-1.5 left-2 text-lg font-bold z-30 leading-none ${
                                isToday
                                  ? 'w-9 h-9 rounded-full bg-[#2d568e] text-white flex items-center justify-center text-base -top-1 -left-1 shadow-md'
              ...
              {!isDragging && (
                <div className="text-center py-4">
                  <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium">
                    💡 Click and drag across dates to add an event
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="flex items-center justify-center h-full bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-600 shadow-sm">
            <div className="text-center py-20">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#2d568e]/10 to-[#2d568e]/20 dark:from-blue-900/20 dark:to-blue-800/20 flex items-center justify-center mx-auto mb-5">
                <Calendar size={36} className="text-[#2d568e] dark:text-blue-400" />
              </div>
      ...
      {/* Context Menu & Event Modal */}
      {contextMenu && (
        <ContextMenu
          x={contextMenu.x}
          y={contextMenu.y}
          condo={contextMenu.condo}
          onClose={() => setContextMenu(null)}
          onAddEvent={(eventType) =>
            setAddEventModal({ isOpen: true, eventType, startDate: null, endDate: null })
          }
        />
      )}
      <AddEventModal
        ...
        condo={selectedCondo || contextMenu?.condo}
        eventType={addEventModal.eventType || 'other'}
        onSave={handleEventSaved}
        startDate={addEventModal.startDate}
        endDate={addEventModal.endDate}
      />
      ...
      <AnimatePresence>
        {hoveredItem && tooltipType === 'booking' && (
          ...
          >
            <div className="flex items-center gap-3 pb-3 border-b border-gray-700/50">
              {hoveredItem.avatar_url ? (
                <img src={hoveredItem.avatar_url} alt="" className="w-10 h-10 rounded-full object-cover ring-2 ring-gray-600" />
              ) : (
                <div
                  style={{
                    background: avatarColors(hoveredItem.guest_name)[0],
                    color: avatarColors(hoveredItem.guest_name)[1]
                  }}
                  className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold ring-2 ring-gray-600"
                >
                  {initials(hoveredItem.guest_name)}
                </div>
              )}
              <div>
                <p className="text-sm font-semibold">{hoveredItem.guest_name || 'Unknown Guest'}</p>
                <span
                  className={`text-[10px] font-semibold uppercase tracking-wider ${
                    hoveredItem.status === 'confirmed' ? 'text-emerald-400' : 'text-amber-400'
                  }`}
                >
                  {hoveredItem.status}
                </span>
            ...
            <div className="space-y-2 mt-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-mono font-semibold">{hoveredItem.booking_code}</span>
              </div>
              {hoveredItem.guest_email && (
                <div className="flex items-center gap-2">
                  <Mail size={14} className="text-gray-500" />
                  <span className="text-xs text-gray-300">{hoveredItem.guest_email}</span>
                </div>
              )}
              <div className="flex items-center gap-2">
                <Calendar size={14} className="text-gray-500" />
                <span className="text-xs text-gray-300">
                  {format(parseISO(hoveredItem.start_date), 'MMM d, yyyy')} → {format(parseISO(hoveredItem.end_date), 'MMM d, yyyy')}
                </span>
              </div>
              {hasEventConflict(hoveredItem) && (
                <div className="flex items-center gap-2 text-red-400">
                  <AlertTriangle size={14} />
                  <span className="text-xs font-semibold">Conflict with event!</span>
                </div>
              )}
            </div>
          </motion.div>
        ...
        {hoveredItem && tooltipType === 'event' && (
          ...
          >
            <div className="flex items-center gap-2 pb-2 border-b border-gray-700/50">
              {(() => {
                const Icon = EVENT_TYPES[hoveredItem.event_type]?.icon || Calendar
            ...
            <div className="mt-2 space-y-1">
              <div className="flex items-center gap-2">
                <Calendar size={14} className="text-gray-500" />
                <span className="text-xs text-gray-300">
                  {format(parseISO(hoveredItem.start_date), 'MMM d')}
                  {' → '}
                  {format(hoveredItem.end_date ? parseISO(hoveredItem.end_date) : parseISO(hoveredItem.start_date), 'MMM d, yyyy')}
                </span>
              </div>
              {hoveredItem.notes && (
                <p className="text-xs text-gray-400 mt-1 italic">"{hoveredItem.notes}"</p>
              )}
            </div>
          </motion.div>
        )}
        {hoveredItem && tooltipType === 'holiday' && (
          ...
          >
            <div className="flex items-center gap-2">
              <span className={`w-2.5 h-2.5 rounded-full ${getHolidayColor(hoveredItem.scope)}`}></span>
              <span className="text-xs font-semibold">{hoveredItem.localName || hoveredItem.name}</span>
            </div>
            <p className="text-[10px] text-gray-400 mt-1 capitalize">
              {hoveredItem.scope === 'national'
                ? '🇵🇭 National Holiday'
                : hoveredItem.scope === 'local'
                ? '🏙️ Iloilo Holiday'
                : '🌍 Worldwide'}
            </p>
          </motion.div>
