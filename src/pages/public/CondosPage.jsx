  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gray-50 pt-6 md:pt-20 pb-24 md:pb-8">
      <div className="max-w-7xl mx-auto px-4 py-4 md:py-8">
        ...
        {condos.length > 0 && <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 transition-opacity duration-300 ${isSearching ? 'opacity-50' : 'opacity-100'}`}>{condos.map(condo => <CondoCard condo={condo} />)}</div>}
      </div>
    </motion.div>
  )
}