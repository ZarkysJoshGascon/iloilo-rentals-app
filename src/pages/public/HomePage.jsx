...
import CondoCard from '../../components/condo/CondoCard'
...
export default function HomePage() {
  ...
  const fetchFeaturedCondos = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        // No limit - shows ALL condos
      
      if (error) {
        console.error('Fetch condos error:', error.message)
        setFeaturedCondos([])
      } else {
        console.log('Fetched condos:', data?.length || 0)
        setFeaturedCondos(data || [])
      }
    } catch (err) {
      console.error('Fetch condos exception:', err)
      setFeaturedCondos([])
    } finally {
      setLoading(false)
    }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('home-condos-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'condos' }, () => {
        console.log('Condo change detected, refetching...')
        fetchFeaturedCondos()
      })
      .subscribe()
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {featuredCondos.map((condo, index) => (
                      <SlideUpSection key={condo.id} delay={index * 0.12}>
                        <CondoCard condo={condo} />
                      </SlideUpSection>
                    ))}
                  </div>
                  <div className="text-center py-8">
                    <p className="text-gray-400 text-xs italic">More premium listings coming soon</p>
                  </div>
                </>
              )}
            </div>
