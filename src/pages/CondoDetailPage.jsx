...
// Custom styles for DatePicker and scrollbar
...
export default function CondoDetailPage() {
  ...
  const CustomDateInput = ({ value, onClick, label }) => (
    <div 
      onClick={onClick}
      className="bg-gradient-to-br from-[#2d568e]/5 to-white rounded-xl p-3 hover:from-[#2d568e]/10 transition-all cursor-pointer border border-[#2d568e]/20 group h-full"
    >
      <div className="text-xs text-[#2d568e] font-semibold mb-1 flex items-center gap-1">
        <CalendarIcon size={12} /> {label}
      </div>
      <div className="w-full text-center font-bold text-gray-800 text-base truncate">
        {value}
      </div>
    </div>
  )
      ...
      <div className="w-1/3 bg-white shadow-2xl flex flex-col" style={{ height: '100vh', position: 'sticky', top: '0' }}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="relative" style={{ zIndex: 5 }}>
            ...
            {showGuestDropdown && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-xl shadow-xl z-30 p-4 space-y-3 animate-in fade-in zoom-in duration-200">
