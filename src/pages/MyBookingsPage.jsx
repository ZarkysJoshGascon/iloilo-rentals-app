    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-pulse">
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="flex-1 p-5 space-y-3">
          <div className="flex justify-between"><div className="space-y-2"><div className="h-5 bg-gray-200 rounded w-40" /><div className="h-4 bg-gray-200 rounded w-24" /></div><div className="h-6 bg-gray-200 rounded w-20" /></div>
          ...
          <div className="flex justify-between pt-3 border-t border-gray-100"><div className="h-3 bg-gray-200 rounded w-24" /><div className="flex gap-3"><div className="h-7 bg-gray-200 rounded w-16" /><div className="h-7 bg-gray-200 rounded w-16" /></div></div>
        </div>
      </div>
    </div>
  )
...
export default function MyBookingsPage() {
  ...
  const cancelBooking = async (bookingId) => {
    if (!confirm('Cancel this booking?')) return; setCancellingId(bookingId)
    try {
      const { error } = await supabase.from('bookings').update({ status: 'cancelled' }).eq('id', bookingId).eq('user_id', user.id)
      if (error) throw error
      toast.success('Booking cancelled'); setBookings(prev => prev.map(b => b.id === bookingId ? { ...b, status: 'cancelled' } : b))
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gray-50 pt-6 md:pt-20 pb-24 md:pb-8">
      <div className="max-w-4xl mx-auto px-4 py-4 md:py-8">
        <div className="mb-6 md:mb-8">
          ...
          <div className="flex gap-2 bg-white rounded-xl p-1.5 border border-gray-100 shadow-sm w-fit overflow-x-auto">
            {tabs.map(tab => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-[#2d568e] text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{tab.label}{tab.id !== 'all' && <span className="ml-1.5 text-xs opacity-70">{bookings.filter(b => b.status === tab.id).length}</span>}</button>))}
          </div>
        </div>
        {loading && <div className="space-y-4">{[1, 2, 3].map(i => <SkeletonBookingCard key={i} />)}</div>}
        {!loading && filteredBookings.length === 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 md:p-12 text-center"><div className="w-16 h-16 bg-[#2d568e]/5 rounded-2xl flex items-center justify-center mx-auto mb-4"><Calendar size={28} className="text-[#2d568e]/40" /></div><h3 className="text-lg font-semibold text-gray-800 mb-1">No bookings found</h3><p className="text-gray-500 text-sm mb-6">{activeTab === 'all' ? "You haven't made any reservations yet." : `No ${activeTab} bookings.`}</p><button onClick={() => navigate('/condos')} className="bg-[#2d568e] text-white px-6 py-2.5 rounded-xl font-medium text-sm hover:bg-[#1e3a5f] transition inline-flex items-center gap-2">Browse Condos <ArrowRight size={16} /></button></div>
        )}
        {!loading && filteredBookings.length > 0 && (
              ...
              <motion.div key={booking.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow group">
                <div className="flex flex-col sm:flex-row">
                  ...
                  <div className="flex-1 p-4 md:p-5 flex flex-col justify-between">
                    <div><div className="flex justify-between items-start gap-2 mb-2"><div><h3 className="font-semibold text-gray-900">{condo?.title || 'Condo Unit'}</h3><div className="flex items-center gap-1 text-gray-500 text-sm mt-0.5"><MapPin size={13} /><span>{condo?.location || 'Iloilo City'}</span></div></div><span className="text-lg font-bold text-[#2d568e]">{formatPrice(booking.total_amount || 0)}</span></div><p className="text-xs text-gray-400 font-mono mb-3">#{booking.booking_code || 'N/A'}</p><div className="grid grid-cols-3 gap-3 mb-3"><div className="bg-gray-50 rounded-xl p-2.5 text-center"><p className="text-xs text-gray-400 mb-0.5">Check-in</p><p className="text-sm font-semibold text-gray-800">{format(new Date(booking.start_date), 'MMM dd')}</p></div><div className="bg-gray-50 rounded-xl p-2.5 text-center"><p className="text-xs text-gray-400 mb-0.5">Check-out</p><p className="text-sm font-semibold text-gray-800">{format(new Date(booking.end_date), 'MMM dd')}</p></div><div className="bg-gray-50 rounded-xl p-2.5 text-center"><p className="text-xs text-gray-400 mb-0.5">Nights</p><p className="text-sm font-semibold text-gray-800">{nights}</p></div></div></div>
                    <div className="flex items-center justify-between pt-3 border-t border-gray-100"><span className="text-xs text-gray-400">Booked {format(new Date(booking.created_at), 'MMM dd, yyyy')}</span><div className="flex items-center gap-3">{(booking.status === 'pending' || booking.status === 'confirmed') && <button onClick={() => cancelBooking(booking.id)} disabled={cancellingId === booking.id} className="text-xs text-red-600 hover:text-red-700 font-medium flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-red-50 transition-colors">{cancellingId === booking.id ? <span className="w-3.5 h-3.5 border-2 border-red-500 border-t-transparent rounded-full animate-spin" /> : <Ban size={12} />}Cancel</button>}<button onClick={() => navigate(`/condo/${booking.condo_id}`)} className="text-xs text-[#2d568e] hover:text-[#1e3a5f] font-medium flex items-center gap-1 px-3 py-1.5 rounded-lg hover:bg-[#2d568e]/5 transition-colors">View Details <ChevronRight size={12} /></button></div></div>
                  </div>
                </div>
              </motion.div>
            )
